...
class SimpleHTMLDomNode
{
    ...
    function innertext()
    {
        if (isset ($this->info[HDOM_INFO_INNER]))
            return $this->info[HDOM_INFO_INNER];
        switch ($this->nodetype)
        {
            case HDOM_TYPE_TEXT :
                return $this->dom->restore_noise($this->info[HDOM_INFO_TEXT]);
    ...
    function outertext()
    {
        ...
        if (isset ($this->info[HDOM_INFO_OUTER]))
            return $this->info[HDOM_INFO_OUTER];
    ...
    protected function seek($selector, & $ret)
    {
        ...
        for ($i = $this->info[HDOM_INFO_BEGIN] + 1; $i < $end; ++ $i)
        {
            ...
            if ($node->nodetype == HDOM_TYPE_ENDTAG)
                continue;
            $pass = true;
            ...
            // compare key
            if ($pass && $key && !(isset ($node->attr[$key])))
            {
                $pass = false;
            }
            // compare value
    ...
    function __set($name, $value)
    {
        ...
        {
            case 'outertext' :
                return $this->info[HDOM_INFO_OUTER] = $value;
            case 'innertext' :
                return $this->info[HDOM_INFO_INNER] = $value;
            case 'plaintext' :
                return $this->dom->restore_noise($this->info[HDOM_INFO_TEXT]);
        }
        if (!isset ($this->attr[$name]))
    ...
    function getElementsById($id, $idx = -1)
    {
